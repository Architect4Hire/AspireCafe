...
namespace AspireCafe.BaristaApi
{
    public class KitchenOrderWorker : BackgroundService
    {
        private readonly IServiceProvider _serviceProvider;
        private readonly ServiceBusClient _serviceBusClient;
        private readonly ILogger<KitchenOrderWorker> _logger;
        private readonly string _topicName = "purchased-orders";
        private readonly string _subscriptionName = "kitchen-orders";

        public KitchenOrderWorker(IServiceProvider serviceProvider, ServiceBusClient serviceBusClient, ILogger<KitchenOrderWorker> logger)
        {
        ...
        private async Task ProcessMessageHandler(ProcessMessageEventArgs args)
        {
            try
            {
                ...
                if (message != null && message.RouteType == RouteType.Kitchen)
                {
                ...
                {
                    // Ignore non-Kitchen orders, just complete the message
                    await args.CompleteMessageAsync(args.Message);
                }
            }
            catch (Exception ex)
